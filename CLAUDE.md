...
`verifySprauthSigned()` always calls `consumeChallenge()` as part of verification — it's built for one-time challenge tokens, not for repeatedly verifying access/refresh tokens. `verifySelfSigned()` is the split-out signature-only half (no Redis interaction) — it's what `/session/refresh` uses to verify refresh tokens repeatedly, and it's the function to reach for if/when access-token auth is added to gate `/session/*` list/end/revoke routes by caller identity.
